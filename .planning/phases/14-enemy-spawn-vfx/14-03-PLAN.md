...
must_haves:
  truths:
    ...
    - "새 층 시작 Room(FloorTransitionSequence Step 4)의 EnemySpawner도 명시적으로 즉시 Activate된다 — 기존 no-op 주석이 실제 호출로 대체된다 (Pitfall 5/Open Question 3 해소)"
    - "RemoveTail()/RemoveHead()로 정리되는 Room/Corridor의 _pendingSpawns/_activatedSections 엔트리가 함께 제거된다 (메모리 정리)"
...
<task type="auto">
  ...
  <action>
    ...
    **5. `FloorTransitionSequence()`** — `_chain.Clear();` 직후에 두 컬렉션도 초기화하고, Step 4 no-op 주석을 실제 호출로 교체:
    ```csharp
    _chain.Clear();
    _activatedSections.Clear();  // Phase 14: 옛 층의 활성화 기록도 함께 초기화
    _pendingSpawns.Clear();      // Phase 14: 옛 층의 대기 스폰 기록도 함께 초기화
    _activeExitCount = 0;
    Debug.Log($"[WorldGenerator] _activeExitCount = {_activeExitCount}");
    ```
    그리고 Step 4 부분을:
    ...
    ```
    로 교체한다 (기존 "의도적 no-op" 주석 2줄을 삭제하고 위 3줄로 대체).

    `newRoom`은 이미 이 메서드 앞부분에서 `GameObject newRoom = portal.StandbyRoom;`으로 선언되어 있으므로 추가 변수 선언 불필요. `TrySpawnEnemies(standbyRoom, FloorManager.CurrentFloor + 1);`는 `TrySpawnExitPortal()`에서 이미 호출되어 `_pendingSpawns[standbyRoom]`에 등록되어 있다 — `standbyRoom`이 곧 `newRoom`이므로 `TryActivateSection(newRoom)`이 그 등록을 그대로 소비한다.
  </action>
  <verify>
    <automated>grep -n "CheckCorridorEntry" Assets/Scripts/World/WorldGenerator.cs && grep -n "TryActivateSection(newRoom)" Assets/Scripts/World/WorldGenerator.cs && grep -c "의도적 no-op" Assets/Scripts/World/WorldGenerator.cs</automated>
  </verify>
  <acceptance_criteria>
    ...
    - `_activatedSections.Clear();`와 `_pendingSpawns.Clear();`가 `FloorTransitionSequence()`의 `_chain.Clear();` 직후에 존재한다
  </acceptance_criteria>
  <done>Corridor 자체 진입 임계값이 Room 경계보다 먼저 감지되고, 새 층 시작 Room도 명시적으로 활성화되며, 정리되는 section의 스폰 기록이 누수 없이 제거됨.</done>
</task>
...
- 실제 동작 검증(포탈 타이밍, 감지 차단)은 14-04의 플레이테스트 체크포인트에서 수행 — 이 플랜 자체는 코드 배선만
</verification>
