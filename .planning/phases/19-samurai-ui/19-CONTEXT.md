...
**이 Phase의 범위:** (1) 패링 모듈(player-side, 실시간 방향성 스윙 + 패링 판정), (2) SAMURAI 보스 FSM(할로우나이트 스타일 평시 콤보 + 간헐적 패링 리듬 구간 + 그로기 게이지), (3) 모듈 선택 UI를 N-way(현재는 2-way)로 확장.
...
### Integration Points
- `Assets/Scripts/Player/Combat/` (신규) — `SamuraiParryModule.cs`(또는 `ParryController.cs`)
- `Assets/Scripts/Enemy/Boss/` (신규) — `SamuraiBoss.cs` (BossEnemyBase 상속)
- `Assets/Scripts/UI/AttackSelectController.cs` (수정) — N-way 확장
