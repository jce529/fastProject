...
- Phase 19 Plan 02 완료(2026-08-07, 병렬 worktree 실행): `CombatModuleRegistry.All`(기본전투모듈/Overclock="Fiora"/사무라이 전투형 모듈="Samurai" 3엔트리) + `CombatModuleSelector`(static 선택 상태, AttackTypeSelector 컨벤션 재사용) 신설, `AttackSelectController`를 하드코딩 2-way(Linear/Fan)에서 레지스트리 순회 데이터 기반 N-way로 전면 재작성(D-13 잠금 표시 포함), `AttackSelectUIBuilder.cs`(Fast/Phase19/Build AttackSelect UI) 멱등적 에디터 도구 신설(씬 반영은 19-06으로 연기, `git diff Assets/Scenes/AttackSelect.unity` 빈 출력 확인). UNLOCK-02/UNLOCK-03 코드 완료로 마킹, 실플레이 검증은 19-06 통합 체크포인트로 이연. 상세는 [19-02-SUMMARY.md](./phases/19-samurai-ui/19-02-SUMMARY.md) 참고. 실행 worktree(`worktree-agent-a8af4b1da1e5fe6b7`)가 세션 시작 시 main보다 크게 뒤처져 있어 `git merge main --ff-only`로 동기화 후 진행 — `state advance-plan` 자동 명령이 STATE.md Current Position의 stale Phase 25 컨텍스트를 오인해 Phase 25 카운터를 잘못 건드려(status를 verifying으로 변경 등) 즉시 되돌리고, 이 Roadmap Evolution 섹션에 수동으로 추가 기록하는 방식으로 대체함 — 병렬 다중 Phase 실행 중에는 `state advance-plan`/frontmatter progress 카운터를 자동 갱신하지 말 것(Current Position이 어느 Phase를 가리키는지 도구가 구분하지 못함)
- Phase 19 Plan 04 완료(2026-08-07, 병렬 worktree 실행): `ParryableProjectile.cs`(IParryable 구현, 죽음 전달은 오직 `OnTriggerEnter2D` 경유 — Pitfall 2, 구르기 i-frame 자동 회피 성립) + `SamuraiBoss.cs`(`BossEnemyBase` 상속, 평시콤보/패링전용타이밍/그로기 3단계 FSM) 신설. 19-CONTEXT.md D-09를 authoritative로 채택해 평시 구간(Normal/ParryWindow) 내내 `IsAlive=true` 유지(FioraBoss의 대시-빈틈 이분법과 다름, 19-RESEARCH.md Pitfall 4의 전제와 명시적으로 다름 — 플랜 목적문 자체가 이 차이를 문서화). `OnDashHit()`이 `_state==Groggy`로만 분기해 평시 타격은 게이지만 채우고(Pitfall 4 방지), 그로기 타격만 7회 처치 카운트(D-11)로 집계 → `Die(...,"Samurai")`가 `BossUnlockManager.Unlock("Samurai")` 트리거. SAMURAI-01/03/04/05 코드 완료로 마킹, 실측 타이밍 튜닝/실플레이는 19-06으로 이연. 상세는 [19-04-SUMMARY.md](./phases/19-samurai-ui/19-04-SUMMARY.md) 참고. 실행 worktree(`worktree-agent-abaeda44adda59c84`)가 세션 시작 시 main보다 1커밋(Phase 19 Plan 01/02 + BossEnemyBase/IParryable 등 이 Plan의 직접 의존성 포함) 뒤처져 있어 `git merge main --ff-only`로 동기화 후 진행. `gsd-tools roadmap update-plan-progress 19` 실행 시 위 항목들에 기록된 것과 동일한 버그로 무관한 Phase 999.2 백로그 "Plans: 3/4 plans executed" 문구가 "3/6"으로 재차 클로버되어 수동 원복 — `state advance-plan`/frontmatter progress 카운터는 이전 항목과 동일한 이유로 자동 갱신하지 않음
- Phase 19 Plan 05 완료(2026-08-07, 병렬 worktree 실행): `SamuraiBossPrefabBuilder.cs`(Fast/Phase19/Build SamuraiBoss Prefab — `BossEnemyPrefabBuilder.cs` D-10 클론/컴포넌트교체/compound-collider 패턴 그대로 미러링, MeleeEnemy.prefab을 SamuraiBoss로 스왑 + 1.5배 스케일/청회색 틴트) + `RoomSamuraiFsmTestBuilder.cs`(Fast/Phase19/Build Room_SamuraiFsmTest — `RoomBossFsmTestBuilder.cs` D-13 컨벤션 그대로, nested SamuraiBoss 인스턴스 포함) 신규 에디터 도구 2종. `DebugSceneBuilder.cs`에 순수 추가 스텝(Room_BossFsmTest 배치 직후, Canvas 구성 이전) 삽입 — `Room_SamuraiFsmTest.prefab` 로드 + `ToSamuraiRoom_Teleporter` pad(`DebugRoomTeleporter`, `targetRoomPrefab`만 배선) + 클론 플레이어에 `DebugCombatModuleSwitcher` 부착, 기존 Room_BossFsmTest 배치/카메라/Canvas 로직은 무변경. `CombatController.cs`에 `DebugSetActiveModule(CombatModuleId)` 1줄 public 래퍼(D-18 대체, 실제 로비 UI 없이 DebugScene에서 즉시 모듈 전환) 추가 — 기존 `BuildModule()` private 팩토리 재사용, 그 외 라인 무변경. `DebugCombatModuleSwitcher.cs` 신규 — 숫자키 1/2/3을 기본전투모듈/Overclock/사무라이 전투형 모듈에 매핑. 이 Plan은 도구만 작성(19-05-PLAN.md 명시 제약) — 어떤 메뉴도 실행하지 않아 `git diff --stat Assets/Prefabs/`, `git diff --stat Assets/Scenes/DebugScene.unity` 모두 빈 출력으로 완료 시점 확인(실제 실행+통합 플레이테스트는 19-06). 상세는 [19-05-SUMMARY.md](./phases/19-samurai-ui/19-05-SUMMARY.md) 참고. 실행 worktree(`worktree-agent-af54c53af825ee4bb`)가 세션 시작 시 main보다 크게 뒤처져 있어(999.4-01까지만 반영, Phase 15/16/18/18.1/19-01~04/25 누락) `git merge main --ff-only`로 동기화 후 진행. `gsd-tools roadmap update-plan-progress 19` 실행 시 이전 Plan들과 동일한 버그로 무관한 Phase 999.2 백로그 "Plans:" 문구가 "5/6"으로 재차 클로버되어 "3/4 plans executed"로 수동 원복 — `state advance-plan`은 병합된 main의 STATE.md Current Position이 Phase 25를 가리키고 있어(다른 병렬 세션의 최신 작업 반영) 실행하지 않음
