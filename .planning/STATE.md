...
- Phase 19 Plan 03 완료(2026-08-07, 병렬 worktree 실행): `SamuraiParryModule`(D-04~D-06 패링 우선 판정 — swing arc 안 `IParryable` OverlapCircle 스캔 후 성공 시 `OnParried()` 호출/return, 없으면 `base.ResolveSwing()`으로 D-02 원샷킬 폴백) 신규 + `CombatController`에 19-RESEARCH.md §1 권장 host-hook 배선: `Awake()`가 `BuildModule(CombatModuleSelector.SelectedModuleId)` 팩토리로 기본전투모듈/Overclock/사무라이 전투형 모듈 3종을 구성(UNLOCK-02), `Update()`는 `_isBusy` 체크 직후 `_activeModule is IRealtimeCombatModule` 조기 반환 분기만 추가해 Overclock의 기존 hold-slowmo→release-resolve 코드는 `git diff` 상 완전히 무변경(INFRA-01). 상세는 [19-03-SUMMARY.md](./phases/19-samurai-ui/19-03-SUMMARY.md) 참고. 이 worktree(`worktree-agent-aab2d0ae318b01310`)도 세션 시작 시 main보다 ~180커밋 뒤처져 있어(999.4까지만 반영, 15/16/18/18.1/19-01/19-02/25 누락) `git merge main --ff-only`로 동기화 후 편집 진행. `state advance-plan` 실행 시 19-02와 동일한 stale-Phase-25 버그가 재현되어(milestone을 v3.1로, status를 verifying으로 되돌림) 즉시 `git checkout -- .planning/STATE.md`로 원복하고 이 항목을 수동 기록하는 것으로 대체 — 자동 카운터 갱신 명령은 계속 회피 필요
