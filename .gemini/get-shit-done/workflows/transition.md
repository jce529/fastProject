...
```bash
TRANSITION=$(node "D:/새 폴더/Projeect_A.E/Projeect_A.E/.gemini/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "D:/새 폴더/Projeect_A.E/Projeect_A.E/.gemini/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "D:/새 폴더/Projeect_A.E/Projeect_A.E/.gemini/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "D:/새 폴더/Projeect_A.E/Projeect_A.E/.gemini/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
